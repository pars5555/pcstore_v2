	...
	<div class="companies_container">
		...
		<div id="companyServiceTab" style="display:none;" class="f_company_tab company_tab">
			...
			<div class="company_container">
				...
				<div class="company_confirm">
					{if $company->getHasPrice()==1 || $company->getPriceId()>0 || $company->getShowPrice()==1}
					<div id="service_price_files_content_{$companyId}">
						<div class="documents-slider">
							...
							<div id="documentSliderWrapper" class="f_document_slider owl-carousel owl-theme text-center">
								{*}
								<div>
									<a href="javascript:void(0);" class="service_price_scroll_left_a" service_company_id="{$companyId}"> <img src = "{$SITE_PATH}/img/price_left_arrow.png"  alt=">"/> </a>
								</div>
								<div>
									<a href="javascript:void(0);" class="service_price_scroll_right_a" service_company_id="{$companyId}"> <img src = "{$SITE_PATH}/img/price_right_arrow.png"  alt="<"/> </a>
								</div>
								{*}
								...
								<a href="{$SITE_PATH}/price/service_zipped_price_unzipped/{$pr->getId()}"> <img src = "{$SITE_PATH}/img/file_types_icons/zip_icon.png"  alt="zip"/> {assign var="uploadDateTime" value = $pr->getUploadDateTime()} <span> {if $uploadDateTime}
									{$uploadDateTime|date_format:"%m/%d"}
									<br />
									{$uploadDateTime|date_format:"%H:%M"}
									{/if} </span> </a>
					...
					<div class="form-group">
						...
						class="service_companies_access_key_inputes text"
						attribute_name_to_translate="placeholder" attribute_phrase_id="600"
						service_company_id="{$companyId}"/>
						<a href="javascript:void(0)" class="f_company_access_key_confirm_btn blue button" service_company_id="{$companyId}"><span class="glyphicon"></span></a>
					</div>
					{/if}
				</div>
				...
				<div class="company-address">
					...
					<span> {foreach from=$addrs item=addr key=index}
						{assign var="region_phrase_id" value=$ns.lm->getPhraseIdByPhraseEn($regions[$index])}
						{$addr}, {$zips[$index]}, {$ns.lm->getPhrase($region_phrase_id)}
						<br/>
						{/foreach} </span>
				</div>
				<div class="company-tel">
					<span> {assign var=phones value=","|explode:$company->getPhones()}
						{foreach from=$phones item=phone}
						{$phone}
						<br/>
						{/foreach} </span>
