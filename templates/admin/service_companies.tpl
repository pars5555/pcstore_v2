<div class="container_service_companies">
    ...
    <a class="button blue" href="{$SITE_PATH}/admin/scompanies/new">New Company</a>
    ...
    {if isset($ns.selectedCompanyDto)}
        <div class="admin_company_settings">
            ...
            <a class="button inline blue" href="{$SITE_PATH}/dyn/admin/do_change_service_company_block_has_price_attributes?company_id={$ns.selectedCompanyDto->getId()}&has_price={if $ns.selectedCompanyDto->getHasPrice()==1}0{else}1{/if}">{if $ns.selectedCompanyDto->getHasPrice()==1}Set Has Not Price{else}Set Has Price{/if}</a>
            <a class="button inline blue" href="{$SITE_PATH}/dyn/admin/do_change_service_company_block_has_price_attributes?company_id={$ns.selectedCompanyDto->getId()}&blocked={if $ns.selectedCompanyDto->getBlocked()==1}0{else}1{/if}">{if $ns.selectedCompanyDto->getBlocked()==1}Unblock Company{else}Block Company{/if}</a>
        </div>
    {/if}
    ...
    <div class="table_striped admin_companies_list">
        ...
        {foreach from=$ns.allCompaniesDtos item=companyDto}
            <a class="table-row" href="{$SITE_PATH}/admin/scompanies/{$companyDto->getId()}" style="{if isset($ns.selectedCompanyDto) && $ns.selectedCompanyDto->getId()==$companyDto->getId()}color:red{/if}">
