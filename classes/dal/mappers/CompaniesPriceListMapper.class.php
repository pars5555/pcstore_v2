...
require_once (CLASSES_PATH . "/dal/dto/CompaniesPriceListDto.class.php");

/**
 *
 * 	@author	Vahagn Sookiasian
 */
class CompaniesPriceListMapper extends AbstractMapper {
    ...
    function __construct() {
        ...
        $this->tableName = "companies_price_list";
    }
    ...
    public static function getInstance() {
        if (self::$instance == null) {
            self::$instance = new CompaniesPriceListMapper();
        }
    ...
    public function createDto() {
        return new CompaniesPriceListDto();
    }
    ...
    public static $GET_COMPANY_LAST_PRICE = "SELECT * FROM `%s` WHERE company_id = %d AND file_ext <> 'zip' ORDER BY id";

    public function getCompanyLastPrices($company_id) {
        $sqlQuery = sprintf(self::$GET_COMPANY_LAST_PRICE, $this->getTableName(), $company_id);
    ...
    public static $GET_COMPANIES_ZIPPPED_PRICES = "SELECT * FROM `%s` WHERE `file_ext`= 'zip' AND `company_id` IN (%s) ";
    ...
    public static $GET_PRICE_COUNT = "SELECT COUNT(id) AS `price_count` FROM `%s` WHERE `company_id` = %d AND `upload_date_time`>= DATE_SUB('%s', INTERVAL %d DAY)";
    ...
    public static $GET_ALL_AFTER_TIME = "SELECT `%s`.*, `companies`.`name` as `company_name` FROM `%s` LEFT JOIN `companies` ON `companies`.`id`=`%s`.`company_id` WHERE `companies`.`hidden`=0 AND `upload_date_time`>= '%s' AND `file_ext`<>'zip'";
    ...
    public function getAllPricesAfterTime($datetime, $companiesIds) {
        ...
        if (!empty($companiesIds)) {
            $sqlQuery.= sprintf(" AND FIND_IN_SET(`company_id`, '%s')", implode(',', $companiesIds));
        }
        return $this->fetchRows($sqlQuery);
    }

    public static $GET_COMPANY_HISTORY_PRICES_ORDER_BY_DATE = "SELECT * FROM `%s` WHERE company_id = '%s' AND file_ext = 'zip' ORDER BY upload_date_time DESC LIMIT %d, %d ";

    public function getCompanyHistoryPricesOrderByDate($company_id, $offset, $limit) {
        $sqlQuery = sprintf(self::$GET_COMPANY_HISTORY_PRICES_ORDER_BY_DATE, $this->getTableName(), $company_id, $offset, $limit);
        $result = $this->fetchRows($sqlQuery);
        return $result;
    }

    public static $GET_COMPANY_TODAY_PRICE_UPLOADED_TIMES = "SELECT COUNT(`id`) as `count` FROM `%s` WHERE company_id = %d AND DATE(upload_date_time) = CURDATE()";
