...
$.fn.imFormValidator = function(){
	...
	$(this).find(":input").each(function(key, item) {
       ...
       case "email":
         ...
         break;
       case "username":
         var status = imValidator.validateString($(item).val(), 4, true);
         break;  
       case "username-email":
         var status = imValidator.validateString($(item).val(), 4, true, true);
         break;   
       case "password":
         var status = imValidator.validateString($(item).val(), 0, false);
         if(passwordFieald && status){
      ...
      if(status !== true){
      	formValidateStatus = false;
				imValidator.showError(item, status);
			}
		}	
	});
...
var imValidator = {
	...
	validateString: function(str, len, allowChars, email){
		...
		if (len) {
			if (str.length < len || str.length > 30) {
	...
	validatePolicy: function(elem){
		if(!$(elem).is(':checked')){
			return "In order to use our services, you must agree to answerrock.com's Terms of use and privacy policy."
			}
		return true;
	},
	showError: function(elem, msg){
		$(elem).parent().append($("<div class='ilyov_validate'>"+msg+"</div>"));
		$(elem).css({"border-color": "#FC5458", 
												             "border-width":"1px", 
												             "border-style":"solid"});
	},
